...
- examples should be of the form:
  - <example>
      ...
      <commentary>
      Since the user is greeting, use the ${ZI} tool to launch the greeting-responder agent to respond with a friendly joke.
      </commentary>
    ...
    - <example>
      ...
      user: "Hello"
      assistant: "I'm going to use the ${ZI} tool to launch the greeting-responder agent to respond with a friendly joke"
      <commentary>
...
`;async function xxB(A,B,Q){let D=Q.length>0?`

IMPORTANT: The following identifiers already exist and must NOT be used: ${Q.join(", ")}`:"",Z=`Create an agent configuration based on this request: "${A}".${D}
